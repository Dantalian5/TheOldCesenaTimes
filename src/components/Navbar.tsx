...
const Navbar = memo(() => {
  return (
    <nav
      className={` sticky top-0 z-10 mx-auto mb-1  hidden max-w-[1285px] bg-white px-5 lg:block lg:px-11 `}
    >
      <ul className="flex items-center justify-center border-y border-b-black-100 border-t-gray-200">
        {Object.entries(navbarItems).map(([section, sectionInfo]) => (
          ...
          >
            ...
            <span className={`ml-1 text-lg text-gray-300`}>{svgArrow}</span>
            <div
              className={`pointer-events-none absolute left-1/2 top-[calc(100%-1px)] w-dvw -translate-x-1/2 gap-x-16 bg-white opacity-0 shadow-menu transition-opacity duration-500 ease-out group-hover:pointer-events-auto group-hover:opacity-100`}
            >
              <span className="mx-auto block w-full max-w-[1285px] gap-x-20  px-5 lg:px-11 ">
                <span className="block w-full border-t border-t-gray-200"></span>
              </span>
